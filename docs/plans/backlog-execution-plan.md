...
Version: 2.4 — B5a-B5c shipped; B5d Scout configuration boundary in progress
...
| B5d / #106 | PR open on `codex/issue-95-b5-scout-config-storage` | [PR #107](https://github.com/diese-tech/Ratatoskr/pull/107) | Async Scout configuration store and SQLite adapter; early interaction acknowledgement with private validation failures; 282 tests locally | Re-run final-head review/CI after fixing the acknowledgement and response-visibility P2s; Scout lifecycle and Postgres implementation remain separate |
| B5 | Active | #100, #102, #104, and #106 are bounded vertical slices | Dedicated Ratatoskr Postgres selected in #1 | Remaining Scout workflow extraction plus schema/verifier/rehearsal/cutover/recovery remain later gates |
...
2. Complete B5d / #106 review and CI without changing Scout behavior, converting the transaction-heavy lifecycle, adding a Postgres driver, provisioning infrastructure, or changing production persistence.
