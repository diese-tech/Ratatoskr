...
Version: 3.1 — B5a-B5g shipped; #90 and #116 merged; PR #120 operational/Scout follow-up merged; B6 reference decision pending
Recorded: September 2, 2026
Updated: September 22, 2026
Repository: [diese-tech/Ratatoskr](https://github.com/diese-tech/Ratatoskr)
Baseline commit: [`47997e5701e1f6ea3cf119ef1a6e8cd064ddcf86`](https://github.com/diese-tech/Ratatoskr/commit/47997e5701e1f6ea3cf119ef1a6e8cd064ddcf86)
...
Separate Scout follow-ups are now merged without changing the B5 sequence: [#90](https://github.com/diese-tech/Ratatoskr/issues/90) shipped fixed three-hour automatic cancellation/finish through [PR #115](https://github.com/diese-tech/Ratatoskr/pull/115) (`b0a728855d37e349ffb7a1da64a1bc2476e78caf`); [#116](https://github.com/diese-tech/Ratatoskr/issues/116) shipped compact cancelled Scout Ops cards and embed boundaries through [PR #117](https://github.com/diese-tech/Ratatoskr/pull/117) (`01f823bbd071265dd64aad68ee793d2d35b5c8ac`). Both issues are closed, and each merge commit passed post-merge Ubuntu/Windows CI. User-provided Discord screenshots show the cancelled, finished, and open Scout Ops card presentations; they do not establish every alert, timing, or recovery acceptance case.

A further operational/Scout follow-up is merged through [PR #120](https://github.com/diese-tech/Ratatoskr/pull/120) (`47997e5701e1f6ea3cf119ef1a6e8cd064ddcf86`), squashed from four reviewed commits: Division Captain role-assignment documentation and a matching `/help` section; an optional `STAFF_OPS_CHANNEL_ID` fallback so operational alerts can reach staff when `staff-ops` is not yet a Ratatoskr-managed resource; persistent `Created by <@user>` Scout setup-creator attribution on Scout Ops cards; and a production-incident fix for `hasSetupScopedControl()`, whose recovery regex could mistake another setup's version number for the setup ID being searched for and cross-adopt that setup's control card. The fix shipped with a regression test reproducing the exact collision (confirmed to fail without the fix and pass with it), and [post-merge Ubuntu/Windows main CI](https://github.com/diese-tech/Ratatoskr/actions/runs/35681846358) passed. This is documentation, operational configuration, and a targeted bug fix; it does not change B5 or B6 sequencing. The production database-level symptom (a stuck Scout setup) was cleared separately by an operator after this fix deployed; that operational cleanup is not part of this repository's history.
...
- Issue #67 currently says it has no blocker, while this plan and tracker #95 require that complete reference. Resolve the discrepancy with the product owner before treating the full scaffold refresh as implementation-ready; this status update does not choose the missing layout.
...
| Three-hour Scout cleanup / #90 | Merged; issue closed; PR and main CI green | [Issue #90](https://github.com/diese-tech/Ratatoskr/issues/90), [PR #115](https://github.com/diese-tech/Ratatoskr/pull/115), b0a7288 | Fixed start+3h deadline; automatic cancel/finish; publication recovery exception; durable Discord repair/audit; final-readiness, notification-lock, overdue-backlog, fair recovery/retry paging, startup ordering, and uncertain-notice preservation; 316 local tests; resolved review threads; Ubuntu/Windows [main CI](https://github.com/diese-tech/Ratatoskr/actions/runs/35138058471) | Screenshot shows automatic-finish attribution, not the exact closure deadline or full live recovery matrix; Railway runtime was not checked in this update |
| Scout Ops embed presentation / #116 | Merged; issue closed; PR and main CI green | [Issue #116](https://github.com/diese-tech/Ratatoskr/issues/116), [PR #117](https://github.com/diese-tech/Ratatoskr/pull/117), 01f823b | Compact count-free cancelled card; one embed for managed Scout Ops cards and two Ops alert types; 336 local tests; Ubuntu/Windows [main CI](https://github.com/diese-tech/Ratatoskr/actions/runs/35176789050) | User screenshot confirms cancelled, finished, and open card readability; alert embeds and other live states remain unverified |
| Captain-role docs, staff-ops fallback, Scout creator attribution, card-recovery fix / PR #120 | Merged; PR and post-merge main CI green | [PR #120](https://github.com/diese-tech/Ratatoskr/pull/120), 47997e5 | Captain role-assignment docs and `/help` section; optional `STAFF_OPS_CHANNEL_ID` staff-ops fallback with the same privacy/delivery checks; persistent `Created by <@user>` Scout Ops card attribution with positive lifecycle assertions; production `hasSetupScopedControl()` recovery cross-adoption fix with a regression test proven to fail without it; 338 local tests; Ubuntu/Windows [post-merge main CI](https://github.com/diese-tech/Ratatoskr/actions/runs/35681846358) | Live Discord delivery of the staff-ops fallback, the creator-attribution presentation, and the cleared production setup are unverified from this repository alone |
...
2. Treat #90, #116, and PR #120 as merged follow-ups, not pending PR work. B5 remains active; if it is set aside, reconcile #67's stated no-blocker status with this plan's full-reference prerequisite before starting B6a. Leave B6b's notification-only role list and B7 design choices open.
...
| 2.9 | Record B5g as merged and #90 as fully specified and locally complete with a fixed three-hour deadline, durable automatic closure/Discord reconciliation, recovery exceptions, fair retry paging, startup ordering, uncertain-notice preservation, final-readiness/notification-lock/backlog safety, and audit evidence | PRs #113/#115, Issue #90, tracker #95, 316 tests and local gate evidence |
| 3.0 | Record #90 and #116 as merged with issue closure and post-merge CI; separate observed Scout Ops card presentation from unverified alert/deadline/recovery behavior; preserve B5 and flag the unresolved #67 reference conflict without choosing a layout | PRs #115/#117, Issues #90/#116/#95, post-merge CI runs 35138058471/35176789050, and user-provided Discord screenshots; documentation only, no B6 implementation or production action |
| 3.1 | Refresh baseline to PR #120's merge (Captain-role docs/`/help`, `STAFF_OPS_CHANNEL_ID` staff-ops fallback, persistent Scout creator attribution, and the production `hasSetupScopedControl()` card-recovery cross-adoption fix); record post-merge main CI; preserve B5/B6 sequencing and tracker #95's open status | PR #120 (47997e5), post-merge main CI run 35681846358, 338 local tests; this update makes no B6 implementation or scope decision and does not mark tracker #95 complete |
