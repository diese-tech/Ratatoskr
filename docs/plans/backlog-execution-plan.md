...
Version: 2.5 — B5a-B5d shipped; B5e Scout notification delivery boundary in progress
...
| B5e / #108 | In progress on `codex/issue-95-b5-scout-notification-delivery` | PR pending | Async notification delivery/recovery store and SQLite adapter; claim/send/confirm ordering; shared operation scope; injected error reporting; 284 tests locally | Open focused PR, then exact-head review and CI; lifecycle scheduling and Postgres remain separate |
| B5 | Active | #100, #102, #104, #106, and #108 are bounded vertical slices | Dedicated Ratatoskr Postgres selected in #1 | Remaining Scout workflow extraction plus schema/verifier/rehearsal/cutover/recovery remain later gates |
...
2. Complete B5e / #108 review and CI without changing notification behavior, splitting scheduling from lifecycle transactions, adding a Postgres driver, provisioning infrastructure, or changing production persistence.
