...
| B5f / #110 | Locally complete on `codex/issue-95-b5-scout-signup-storage` | PR pending | Async signup/working-roster store and SQLite adapter; reaction, restart and member-refresh conversion; injected card/error dependencies; 289 tests locally | Open focused PR, then exact-head review and CI; published lifecycle and Postgres remain separate |
