...
## Cancellation, published replacements and finishing

- Use **Cancel setup** on an open or roster-ready card, including a recovered past-dated card. `/scout cancel` in `#scout-ops` also immediately lists all open or roster-ready postings you can manage. Entries show division, scheduled time and status; select one and confirm. Larger lists have Previous/Next pages. Published, cancelled and still-posting setups are excluded.
...
- Published Scout Ops cards now offer those same **Swap players** and **Replace player** controls. They edit the original roster even when a legacy roster lives in a results channel.
- Staff may finish a published scout before its automatic deadline with **Finish scout** on the Scout Ops card and a private confirmation. The bot records the staff member and completion time, marks both posts finished, and removes editing controls. Signups, roster assignments and historical readiness remain stored. Finishing is final; previously opened player-edit forms also become inert. For manual finish, publication or roster delivery must be settled first.
- Manual deletion does not close stored work. Cancel or finish officially even if an original post has already been deleted. Confirmed deletion counts as post cleanup complete; permission failures leave cleanup pending with **Retry post cleanup** on the staff card and a startup retry. The stored cancellation/completion already takes effect while cleanup is pending.

### Automatic three-hour deadline

- Every Scout setup has a fixed deadline at its scheduled start time plus three hours. Signup, roster, swap, and replacement activity does not extend it. Ratatoskr processes due setups on the first worker pass at or after that deadline (normally within about 15 seconds), including overdue setups after a restart.
- At the deadline, `open` and `roster_ready` setups are automatically cancelled; `published` setups are automatically finished, even when a roster update or Discord post repair is pending. A two-game scout closes as one setup. The final cards and posts identify Ratatoskr's automatic action, remove active controls, and skip remaining scheduled notifications. No warning, player/organizer ping, or new public message is sent.
- `posting` and `posting_failed` setups are not force-closed. Ratatoskr continues publication recovery; if it remains unresolved, it sends a deduplicated staff-ops alert. A Discord edit failure after automatic closure does not reopen the setup: the database remains final, and Ratatoskr retries post/card reconciliation during normal operation and after restart. If a staff alert cannot be delivered, it remains pending for retry. Restore Discord access and inspect the alert reference and Railway logs rather than repeating a player change.
