...
import type { ScoutLifecycleCleanup, ScoutRosterSlotRecord, ScoutSetup, ScoutSignup } from '../db/types.js';
...
export interface ScoutReadinessCardStore {
  ...
  getLifecycleCleanup(setupId: number): Promise<ScoutLifecycleCleanup | undefined>;
  ...
  patchSnapshotIfStatus(setupId: number, expectedStatus: ScoutSetup['status'], snapshotJson: string): Promise<boolean>;
